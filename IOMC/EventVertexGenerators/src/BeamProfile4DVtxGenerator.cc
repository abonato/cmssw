...
// $Id: BeamProfile4DVtxGenerator.cc , Created on 01/11/2013 A. Bonato $

#include "IOMC/EventVertexGenerators/interface/BeamProfile4DVtxGenerator.h"
...
BeamProfile4DVtxGenerator::BeamProfile4DVtxGenerator(const edm::ParameterSet & p) :
  ...
  edm::LogInfo("VertexGenerator") << "BeamProfile4DVtxGenerator: with "
				  << "beam along eta = " << fEta 
				  << " (Theta = " << fTheta/deg 
				  << ") phi = " << fPhi/deg 
				  << ") psi = " << fPsi/deg 
...
BeamProfile4DVtxGenerator::~BeamProfile4DVtxGenerator() {
  delete fRandom;
}


//Hep3Vector * BeamProfile4DVtxGenerator::newVertex() {
HepMC::FourVector* BeamProfile4DVtxGenerator::newVertex() {
  double aX, aY;
  if (ffile) {
...
*/
  ...
  LogDebug("VertexGenerator") << "BeamProfile4DVtxGenerator: Vertex created "
			      << "at (" << xp << ", " << yp << ", "
			      << zp << ", " << fTimeOffset << ")";
  return fVertex;
}

void BeamProfile4DVtxGenerator::sigmaX(double s) { 

  if (s>=0) {
    fSigmaX = s; 
  } else {
    edm::LogWarning("VertexGenerator") << "Warning BeamProfile4DVtxGenerator:"
...
void BeamProfile4DVtxGenerator::sigmaY(double s) { 
...
void BeamProfile4DVtxGenerator::eta(double s) { 
  fEta   = s; 
  fTheta = 2.0*atan(exp(-fEta));
}

void BeamProfile4DVtxGenerator::setType(bool s) { 
